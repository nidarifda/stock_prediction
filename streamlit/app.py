...
# ---------------------------------------------------------------
# Top bar (segmented control fallback)
# ---------------------------------------------------------------
...
# Right side: boxed Affiliated Signals with dual-line sparklines
with right:

  def sparkline(values: pd.Series | np.ndarray,
                key: str,
                values2: pd.Series | np.ndarray | None = None):
    """Tiny chart: primary + optional secondary (e.g., moving average)."""
    x = list(range(len(values)))
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=x, y=values, mode="lines", line=dict(width=2), name="raw"))
    if values2 is not None:
      fig.add_trace(go.Scatter(x=x, y=values2, mode="lines", line=dict(width=2), name="smooth"))
  ...
  def signals_panel(title: str, names: list[str], key_prefix: str):
    # Whole block in one card (like your screenshot)
    ...
    for i, nm in enumerate(names):
      vals = aff[nm].tail(40).reset_index(drop=True)
      smooth = vals.rolling(8, min_periods=1).mean()  # secondary line (orange by default)
      delta = float(vals.iloc[-1] - vals.iloc[-2]) if len(vals) > 1 else 0.0
      ...
      with c2:
        sparkline(vals, key=f"sp_{key_prefix}_{i}", values2=smooth)
...
with st.expander("How to wire real data (read me)"):
  st.markdown(
    ...
    - The right-panel sparklines plot **raw** (blue) and a **rolling mean** (orange) for clarity.
    """
  )
