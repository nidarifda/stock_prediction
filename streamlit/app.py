...
# ────────────────────────────────────────────────────────────────────────────────
# CSS – enhanced for Bloomberg-style look
# ────────────────────────────────────────────────────────────────────────────────
st.markdown(f"""
<style>
:root {{
...
/* Title */
.app-header {{ display:flex; align-items:center; margin:2px 0 12px 0; }}
.app-header .title {{ color:{TEXT}; font-size:32px; font-weight:800; letter-spacing:.2px; }}
...
.toprow .btn-wrap .stButton>button {{
  ...
  color:white; font-weight:700; box-shadow:0 0 14px rgba(73,107,255,.45);
}}
.toprow .btn-wrap .stButton>button:hover {{
  background:linear-gradient(90deg,#00C2FF 0%,#496BFF 100%) !important;
}}

/* Selectboxes & Radios unified height */
...
[data-testid="stRadio"], [data-testid="stRadio"] * {{ color:{TEXT} !important; }}
...
prices=load_prices()
left, mid, right = st.columns([0.8,1.6,1.3],gap="small")
...
with left:
    ...
    if prices.empty:
        ...
        for t in DISPLAY_ORDER:
            ...
            s=prices[t].dropna()
            if len(s)<2: continue
            last=s.iloc[-1]; chg=(s.iloc[-1]-s.iloc[-2])/s.iloc[-2]*100
...
with mid:
    ...
    with col1: st.selectbox("",DISPLAY_ORDER,index=0,label_visibility="collapsed")
    with col2: st.radio("",["Next day","1D","1W","1M"],horizontal=True,index=1,label_visibility="collapsed")
    with col3: st.selectbox("",["LightGBM","XGBoost","LSTM"],index=0,label_visibility="collapsed")
    ...
    # Chart
    if not prices.empty and "NVDA" in prices.columns:
        s=prices["NVDA"].dropna()
        fig=go.Figure()
    ...
    with tab2:
        st.markdown("<div class='card'><b>Bias:</b> <span style='color:#FFCE6B'>Mild long</span><br>"
                    "Entry 423.00<br>Target 452.00</div>",unsafe_allow_html=True)
...
with right:
    ...
    if not prices.empty:
        for t in ["TSMC","ASML","CDNS","SNPS"]:
            ...
            color=GREEN if chg>0 else ORANGE
            st.markdown(f"<b style='color:{color}'>{PRETTY[t]} {chg:+.2f}%</b>",unsafe_allow_html=True)
