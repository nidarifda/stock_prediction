...
st.markdown(
    ...
    <style>
      ...
      # ⬇️ add once near the top (after your color vars)
st.markdown("""
<style>
  /* Scope just the signals area so we don't affect the rest of the page */
  ...
  /* Style the bordered Streamlit container like a card */
  .signals-scope [data-testid="stVerticalBlockBorderWrapper"]{
    background: var(--card);
    border: 1px solid rgba(255,255,255,.08);
    border-radius: 12px;
    box-shadow: 0 6px 18px rgba(0,0,0,.22);
    padding: 12px 14px;        /* inner padding for the card */
    margin-top: 6px;            /* sits close to controls above */
    /* optional: lock height to match a chart/watchlist
    min-height: 360px;
    */
  }

  /* Title + divider used inside your card */
